/* ════════════════════════════════════════════════════════════════════
   SUSTAINABILITY — Environmental & Governance Hero behavior
...
(function () {
  ...
  const lines = hero.querySelectorAll('.sush-line-inner');
  const desc = hero.querySelector('.sush-desc');
  const stats = hero.querySelector('.sush-stats');
  const ctaRow = hero.querySelector('.sush-cta-row');
  const bgLighting = hero.querySelectorAll('.sush-glow-wrap, .sush-rays');
  const particlesLayer = hero.querySelector('.sush-particles');
  ...
  if (ctaRow && window.matchMedia('(hover: hover)').matches) {
    const btn = ctaRow.querySelector('.sush-cta');
    if (btn) {
  ...
  if (ctaRow) {
    ctaRow.addEventListener('click', (e) => {
      const btn = e.target.closest('.sush-cta');
      if (!btn) return;
      const rect = btn.getBoundingClientRect();
      const size = Math.max(rect.width, rect.height) * 1.3;
      const ripple = document.createElement('span');
      ripple.className = 'sush-ripple';
