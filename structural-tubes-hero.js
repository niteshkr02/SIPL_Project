/* ════════════════════════════════════════════════════════════════════
   STRUCTURAL TUBES & PIPES HERO — vanilla JS, no animation library.
   Owns: entrance reveal (single .stp-loaded class flip, CSS does the
   ...
   is set. Mirrors heavy-structural-hero.js / concrete-reinforcement-hero.js.
   ════════════════════════════════════════════════════════════════════ */
(function () {
  'use strict';

  const hero = document.querySelector('.stp-hero');
  if (!hero) return;

  const bg = hero.querySelector('.stp-bg');
  const frame = hero.querySelector('.stp-visual-frame');
  const content = hero.querySelector('.stp-content');

  const reduceMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
  const canHover = window.matchMedia('(hover: hover)').matches;

  requestAnimationFrame(() => {
    hero.classList.add('stp-loaded');
  });
